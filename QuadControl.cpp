...
VehicleCommand QuadControl::GenerateMotorCommands(float collThrustCmd, V3F momentCmd)
{
	...
	cmd.desiredThrustsN[0] = (f1 + f2 + f3 + f4) / 4.f;  // front left  - f1
	cmd.desiredThrustsN[1] = (-f1 + f2 - f3 + f4) / 4.f; // front right - f2
	cmd.desiredThrustsN[2] = (f1 - f2 - f3 + f4) / 4.f; // rear left   - f4
	cmd.desiredThrustsN[3] = (-f1 - f2 + f3 + f4) / 4.f; // rear right  - f3
...
V3F QuadControl::RollPitchControl(V3F accelCmd, Quaternion<float> attitude, float collThrustCmd)
{
	...
	if (collThrustCmd > 0) {
		...
		pqrCmd.x = (R(1, 0) * b_x_ - R(0, 0) * b_y_) / R(2, 2);
		pqrCmd.y = (R(1, 1) * b_x_ - R(0, 1) * b_y_) / R(2, 2);
	}
	else {
		pqrCmd.x = 0.0;
		pqrCmd.y = 0.0;
...
V3F QuadControl::LateralPositionControl(V3F posCmd, V3F velCmd, V3F pos, V3F vel, V3F accelCmdFF)
{
	...
	accelCmd = kpPos * (posCmd - pos) + kpVel * (horVel - vel) + accelCmd;
...
float QuadControl::YawControl(float yawCmd, float yaw)
{
	...
	if (yawCmd > 0) {
		yaw_cmd_lim = fmodf(yawCmd, 2 * F_PI);
	}
	else {
		yaw_cmd_lim = -fmodf(-yawCmd, 2 * F_PI);
	}

	float yaw_err = yaw_cmd_lim - yaw;
